...
- `figma-spec-component-build`：**规范页怎么做成可切组件。** 变体轴、TEXT 属性、原稿还原 vs Sketch 垃圾、目录卡。选择器 / 表单行 / 步骤条及后续切片都走它。
- `c-end-visual-craft`：C 端视觉质感。不重造组件；只做氛围 / 主视觉 / 去 AI 感。
- `claims-visual-guard`: 理赔工具稿护栏。乱码、空壳、图层名错误一律丢弃。

出 Figma 稿时顺序：`alipay-app-design-system` → `figma-spec-component-build` → `figma-use` / `figma-generate-library` → 需要质感时再开 `c-end-visual-craft`。
