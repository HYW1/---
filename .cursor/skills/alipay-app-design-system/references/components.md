...
Source file key (unpublished Copy): `4Q2KGg9fstrN2BWCwYChzi`

Copy 文件里的 key **不能** `importComponentSetByKeyAsync`（未发布）。理赔文件 `NiXwro23h7CvvWT7GOb1ep` 跨文件导入必须用已发布团队库「信用卡基础组件」——组件名仍是 `操作/按钮 Button`，变体轴与 L/XL 尺寸与 Copy 一致，走 APP token。这不是信用卡业务规范。
...
禁止手绘 TopBar / 主按钮 / Switch / Checkbox。先导入实例，再改文案与变体。

## Published keys (use these in the claims file)

| Component | Published key | Copy key (same file only) |
|---|---|---|
| 操作/按钮 Button | `6da3c2344daa541d3323897a609ef2161b943234` | `ea7ef268464a2c5496b5283d6e4c550fbb5b638b` |
| 操作/按钮组 ButtonGroup | `0ae9085c2ede5c93c3588c19ea0b6c6d9dd439e5` | `460cfd6d0f74dbb659d4d6b951b805ef15de713c` |
| 展示/页脚 PageFooter | `20e518c6636ea3b981f6245182cd22fe3df5859c` | `a1d8fc65d6c59f6557464e33e3f8123d5f82fc3f` |

Button 文本属性：`按钮文案`（默认「行动文案」）。成对吸底用 ButtonGroup `类型=横向两个`（两个 339×98 的 L 按钮），不要手拼。

文字链「取消申请」用 PageFooter `文字可点击`，不要伪造 Text Button 胶囊。
...
Button dimensions (750 canvas, do not ÷2 in Figma). **Published library live instances** (default label「行动文案」):

- XS: `144 × 48`
- S: `172 × 60`（hug；Copy 文件里同变体是 `176 × 60`）
- M: `192 × 80`（hug；Copy 文件里同变体是 `176 × 80`）
- L: `718 × 98`
- XL: `718 × 128`

Use L for the standard fixed bottom primary action. Use ButtonGroup instead of manually aligning multiple buttons. Pair variant `类型=横向两个` is `742 × 146` with two `339 × 98` L buttons.
