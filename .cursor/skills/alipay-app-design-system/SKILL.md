...
description: 支付宝 APP 基础规范 + 历史稿可迁移体验模式。适用于任意产品设计需求（不限单一业务）。对本仓库任何 Figma / UX 请求默认启用：出稿、改组件、写流程、参考历史稿。
version: 1.0
---
...
2. **Load Figma skills** — `figma-use`；整页再用 `figma-generate-design`。组件/变体再加载 `figma-generate-library`。
...
详见 [references/layout-and-content.md](references/layout-and-content.md)、[references/design-principles.md](references/design-principles.md)。

## Experience patterns from 历史稿 (可迁移)

完整地图见 [references/pattern-archive.md](references/pattern-archive.md)。做新需求时按类型复用：

| 模式族 | 何时用 | 详情 |
|---|---|---|
| 阵地 / 门户 | 服务首页、多入口聚合 | [references/portal-and-cards.md](references/portal-and-cards.md) |
| 个性化卡片 | 进度、引导、权益、兜底运营位 | 同上 |
| 状态 / 结论卡 | 审核结果、拒赔解读、多进度 | [references/status-and-result-cards.md](references/status-and-result-cards.md) |
| 申请链路模块 | 说明→选择→上传→确认→签名→完成 | [references/application-flow-modules.md](references/application-flow-modules.md) |
...
- 使用 WorldFirst / Antom / AlipayHK / 飓创当支付宝 APP 基础库
- 把信用卡**业务稿**当理赔 Kitchen。团队库名「信用卡基础组件」若组件为 `操作/按钮` 且走 APP token，则是已发布的 APP 基础库，跨文件导入必须用它（Copy 文件未发布）

## Figma canvas scale

源库与页面稿都按 **750 宽**画。不要在 Figma 里把尺寸 ÷2。÷2 只用于代码/375 逻辑宽。

Button L = `718 × 98`。成对吸底用 `操作/按钮组 ButtonGroup`，不要手拼两个矩形。

## Reference files

- [references/tokens.md](references/tokens.md)
- [references/components.md](references/components.md)
- [references/layout-and-content.md](references/layout-and-content.md)
- [references/design-principles.md](references/design-principles.md)
- [references/pattern-archive.md](references/pattern-archive.md)
- [references/portal-and-cards.md](references/portal-and-cards.md)
- [references/status-and-result-cards.md](references/status-and-result-cards.md)
- [references/application-flow-modules.md](references/application-flow-modules.md)
